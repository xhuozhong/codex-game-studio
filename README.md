...
> **当前版本：2.0.0-source.1（源码整理版）。** 11 个 Skill 正文来自本机已安装集合；未成功取得原始 `codex-game-studio-2.0.0-offline.zip`，因此不宣称本仓库与原 ZIP 逐字节一致。安装脚本和文档已重新整理。来源与许可状态见 [NOTICE](NOTICE.md)。
...
本仓库提供可阅读、可下载的源码整理版；原 ZIP 的完整性和许可证尚未核验，不把它标记成原始 2.0.0 正式发行包。未发布 Release ZIP 时，可直接使用 GitHub 自动提供的源代码 ZIP。
